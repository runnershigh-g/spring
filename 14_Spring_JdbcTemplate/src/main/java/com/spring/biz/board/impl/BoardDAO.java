...
public class BoardDAO {
	...
	public void insertBoard(BoardVO vo) {
		...
		try {
			...
			stmt = conn.prepareStatement(BOARD__INSERT);
			stmt.setString(1, vo.getTitle());
			stmt.setString(2, vo.getWriter());
			stmt.setString(3, vo.getContent());
			
			stmt.executeUpdate(); // insert, update, delete == executeUpdate 사용
		} catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void updateBoard(BoardVO vo) {
		...
		try {
			conn = JDBCUtil.getConnection();
			stmt = conn.prepareStatement(BOARD__UPDATE);
			stmt.setString(1, vo.getTitle());
		...
		public void deleteBoard(BoardVO vo) {
			...
			try {
				conn = JDBCUtil.getConnection();
				stmt = conn.prepareStatement(BOARD__DELETE);
		...
		public BoardVO getBoard(BoardVO vo) {
			...
			BoardVO board = null;
				...
				try {
					stmt = conn.prepareStatement(BOARD__GET);
					stmt.setInt(1, vo.getSeq());
		...
		// 글 목록 조회
		public List<BoardVO> getBoardList(BoardVO vo) { // 파라미터값 사용X
			System.out.println("===> JDBC로 getBoardList() 실행");
			List<BoardVO> list = null;

			try {
					conn = JDBCUtil.getConnection();
					stmt = conn.prepareStatement(BOARD__LIST);
